...
SetODEs_SEIR=function(t,y,p){
  ...
  with(as.list(p),{
    ...
    dS.dt = -(b1*I1+b2*I2+b3*I3)*S*(1 + seas.amp*cos(2*pi*(t-seas.phase)/365))
    dE.dt=(b1*I1+b2*I2+b3*I3)*S*(1 + seas.amp*cos(2*pi*(t-seas.phase)/365))-a*E
...
GetModelParams = function(input){
  ...
  b3=input$b31*b1
  b=c(b1,b2,b3)
  pModel=c(b=b,seas.amp=seas.amp,seas.phase=seas.phase,pModel)
...
SimSEIR = function(input){
  ...
  if(seas.amp>0){
    ...
    Ro.seas=GetRo_SEIR_Season(pModel,N)
