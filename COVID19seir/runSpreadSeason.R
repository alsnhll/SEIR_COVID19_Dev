...
b1= 0.2 #Transmission rate (mild infections)
...
seas.amp=0.5; #relative amplitude of seasonal fluctuations, in [0,1]
seas.phase=0/365; #phase of seasonal fluctuations, measuered in months relative to time zero when peak will occur (0=time zero, 1 = 1 year from time zero). Can be negative

#Put these into an input structure
input=list("IncubPeriod"=IncubPeriod,"DurMildInf"=DurMildInf,"FracSevere"=FracSevere,"FracCritical"=FracCritical,"ProbDeath"=ProbDeath,"DurHosp"=DurHosp,"TimeICUDeath"=TimeICUDeath,"b1"=b1,"b21"=b21,"b31"=b31,"LogN"=LogN,"Tmax"=Tmax,"InitInf"=InitInf,"yscale"=yscale,"seas.amp"=seas.amp, "seas.phase"=seas.phase)
...
if(seas.amp>0){
  ...
  beta.data=select(out.df,c("time"))
  beta.data$b1=b1*(1 + seas.amp*cos(2*pi*(beta.data$time-seas.phase)/365))
  beta.data$b2=b21*b1*(1 + seas.amp*cos(2*pi*(beta.data$time-seas.phase)/365))
  beta.data$b3=b31*b1*(1 + seas.amp*cos(2*pi*(beta.data$time-seas.phase)/365))
  beta.data=melt(beta.data,id="time")
  
  p.beta=plot_ly(data = beta.data, x=~time, y=~value, color=~variable, type='scatter', mode='lines')
  
  p.beta=layout(p.beta,xaxis=list(title="Time since introduction (days)"),yaxis=list(title=paste("Transmission rate (per day)"),type=input$yscale),
                annotations=list(text=HTML(paste("R", tags$sub(0),tags$sup("now"),'=',formatC(Ro.Season$Ro.now,digits=2),"<br>R", tags$sub(0),tags$sup("max"),'=',formatC(Ro.Season$Ro.max,digits=2),"<br>R", tags$sub(0),tags$sup("min"),'=',formatC(Ro.Season$Ro.min,digits=2))),
                                 showarrow=FALSE,xref="paper",xanchor="left",x=0.95, yref="paper", yanchor="center",y=0.4, align="left"))
  p.beta
